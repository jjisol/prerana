...
@csrf_exempt
def instructorApproval(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if instructor is not None:
                ...
                form = InstructorApplicationForm()
                return render(request, 'instructor_approval.html', {'message': '입력하신 정보에 해당하는 강사정보를 찾지 못했습니다.', 'form': form})
        else:
            form = InstructorApplicationForm()
            return render(request, 'instructor_approval.html', {'message': '다시 입력해주세요.', 'form': form})
    else:
        form = InstructorApplicationForm()
        return render(request, 'instructor_approval.html', {'message': '다시 시도해주세요.', 'form': form})
